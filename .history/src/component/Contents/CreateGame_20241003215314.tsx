...
const CreateGame: React.FC = () => {
  ...
  return (
    <div className="p-6">
      {/* Conditional rendering for header and table */}
      {games.length === 0 ? (
        // If no games exist, show logo and message to create a game
        <div className="flex flex-col items-center justify-center h-screen">
          <img src="/path/to/gamepass-logo.png" alt="GamePass Logo" className="w-32 h-32" />
          <p className="text-lg font-bold mt-4">Create Your First Game</p>
          <Button className="mt-6 text-sm" onClick={() => setIsCreateGameModalOpen(true)}>
            Create Game
          </Button>
        </div>
      ) : (
        // If games exist, show header and table
        <>
          <Button className="absolute right-4" onClick={() => setIsCreateGameModalOpen(true)}>
            Create Game
          </Button>
          br
          <Table
      ...
      {isCreateGameModalOpen && (
        <Modal onClose={() => setIsCreateGameModalOpen(false)}>
          <h2 className='text-black text-bold' style={{ fontSize: "20px" }}>Create a New Game</h2>
          <Input
            label="Game Name"
            value={newGame.name}
          ...
          />
          <Button className='ml-10' onClick={handleCreateGame}>Submit</Button>
        </Modal>
      )}
